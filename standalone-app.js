/* KITU standalone bundle - para abrir index.html directamente (file://) */
(function(){
// ============================================
...
const CONFIG = {
    ...
    DATABASE_MODE: 'local' // file:// no soporta módulos ES ni Supabase,
};
...
class Store {
    constructor() {
        ...
        if (window.CONFIG.DATABASE_MODE === 'supabase') {
            this.initSupabase();
        }
    }
    ...
    async fetchInitialData() {
        if (window.CONFIG.DATABASE_MODE !== 'supabase' || !this._state.user) return;
            ...
            const { data, error } = await this.supabase.auth.signUp({
                ...
                options: {
                    data: {
                        ...
                        country_code: window.CONFIG.ACTIVE_REGION
                    }
                }
            });
...
store.CONFIG = window.CONFIG;
store.getActiveSettings = window.getActiveSettings;
